...
func getIndentLevel(items []lex.Item) int {
	itemsLength := len(items)
	foundIndentLevel := 0
			...
			if current+1 < itemsLength && (items[current+1].IsTab() || items[current+1].IsSpace()) {
				...
				if indentLevel < foundIndentLevel {
					if foundOrderedListItem(items[current+1:]) ||
						...
						current = current + 1
						for current < itemsLength {
							if !(items[current].IsTab() || items[current+1].IsSpace()) {
								break
							}
							current++
						}
				...
				} else if foundNestedListStart != 0 {
					foundNestedListEnd = current
				}
			} else {
				foundNestedListEnd = current
