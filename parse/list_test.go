...
func TestIsUnorderedList(t *testing.T) {
	var testCases = []struct {
		...
		{testdata.UnorderedListBasic, 16, true},
	...
	for _, tc := range testCases {
		t.Run(tc.source, func(t *testing.T) {
			...
			found, end := p.matchesUnorderedList(0)
			if tc.end != end {
...
func TestIsOrderedList(t *testing.T) {
	...
	testCases := []struct {
		...
		{testdata.OrderedListBasic, 16, true},
	...
	for _, tc := range testCases {
		...
		found, end := p.matchesOrderedList(0)
		if tc.end != end {
			t.Errorf("expected %s end to be %d. got %d", tc.source, tc.end, end)
		}
		if tc.found != found {
...
// 		start, end := p.findOrderedListBoundaries(0)
// 		if tc.start != start {
// 			t.Errorf("expected \"%s\" start to be %d. got %d", tc.source, tc.start, start)
// 		}
// 		if tc.end != end {
